...
test("GraphQL WebSocket authenticates all roles and returns state-change payloads", { timeout: 5_000 }, async (t) => {
  ...
  const incidents = client("read").iterate({ query: "subscription { incidentChanged { id status affectedServices baseline { requestCount errorRate p95LatencyMs } createdAt acknowledgedAt resolvedAt } }" });
  ...
  await operations.observe({ source: "gateway", destination: "inventory-api", status: 200, durationMs: 10 });
  assert.equal((await health.next()).value.data.serviceHealthChanged.health, "HEALTHY");
  ...
  assert.deepEqual(service, { id: "gateway", name: "gateway", owner: "Uttam Bhattarai", health: "CRITICAL", metrics: { requestCount: 2, errorRate: 0.5 } });
  ...
  assert.deepEqual(incident.baseline, { requestCount: 1, errorRate: 0, p95LatencyMs: 10 });
