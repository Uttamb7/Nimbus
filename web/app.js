...
function renderFeeds() {
  ...
  $("#incidents").innerHTML=state.incidents.length?state.incidents.map((incident)=>`<div class="feed-item"><strong><span class="status critical">${incident.severity}</span> ${safe(incident.title)}</strong><time>${new Date(incident.createdAt).toLocaleTimeString()}</time><p>${safe(incident.triggerCondition)} · affected: ${incident.affectedServices.map(safe).join(", ")}</p><p>${incident.evidence.requestCount==null?"Historical incident · no captured metrics":`${incident.evidence.requestCount} requests · ${pct(incident.evidence.errorRate)} errors · ${fmt(incident.evidence.p95LatencyMs)} ms p95 · ${pct(incident.evidence.availability)} available`}</p>${incident.status!=="RESOLVED"?`<div class="feed-actions">${incident.status==="OPEN"?`<button data-ack="${incident.id}">Acknowledge</button>`:""}<button data-resolve="${incident.id}">Resolve</button></div>`:""}</div>`).join(""):"No incidents detected.";
