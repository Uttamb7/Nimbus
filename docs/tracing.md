...
Each incident row can also request its related live traces. The authenticated `incidentTraces(id: "...", limit: 5)` query searches the incident's suspected service from five minutes before detection through its resolution time, or through the current time while it remains active. The console isolates lookup failures to that investigation view. This is measured diagnostic evidence from Jaeger, not a persistent incident snapshot; an older incident can have no results after Jaeger restarts or drops in-memory data. Nimbus uses the Jaeger UI's internal JSON search endpoint, so a Jaeger upgrade can require an adapter update.
