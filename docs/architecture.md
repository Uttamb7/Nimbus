...
The control plane serves a web console on port 4000. Its topology, service metrics, incidents, fault actions, and audit stream use the same GraphQL API. Authenticated GraphQL subscriptions at `/graphql/ws` notify the console after health updates and successful incident/audit writes. The console refetches state on notifications and reconnection, retaining polling during connection or query failures. Bounded process-local queues keep slow subscribers from blocking telemetry ingestion; notifications do not replace durable history. See [live updates](subscriptions.md) for the protocol, authentication, and limits.
