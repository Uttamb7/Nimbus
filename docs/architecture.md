...
Incidents and append-only audit events are stored in local PostgreSQL through versioned migrations. GraphQL reads that persistent history, and mutation audit writes finish before success is returned. Telemetry samples, calculated service health, and observed topology remain bounded in control-plane memory. See [history storage](history-storage.md) for local migration, inspection, and reset operations.
